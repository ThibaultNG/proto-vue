...
export const useWeatherStore = defineStore("weatherStore", () => {
	...
	watchEffect(() => {
		if (latitude.value ? latitude.value : 90 >= 90) {
			latitude.value = 90;
		} else if (latitude.value ? latitude.value : -90 <= -90) {
			latitude.value = -90;
		}
		if (longitude.value ? longitude.value : 180 >= 180) {
			longitude.value = 180;
		} else if (longitude.value ? longitude.value : -180 <= -180) {
			longitude.value = -180;
		}
	});
	watchEffect(() => {
